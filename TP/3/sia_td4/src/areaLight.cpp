...
Photon AreaLight::samplePhoton() const
{
    Point3f pos = this->position()
            + this->size()[0]*this->uVec() * Eigen::internal::random<float>(-0.5,0.5)
            + this->size()[1]*this->vVec() * Eigen::internal::random<float>(-0.5,0.5);

    Vector3f v = Warp::squareToCosineHemisphere(pos);

    Color3f pow = M_PI*this->size()[0]*this->size()[1]*m_intensity;

    //return Photon(pos, v, pow);
