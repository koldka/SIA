...
Photon Scene::samplePhoton() const {
    ...
    int x = 0;
    for(LightList::const_iterator it=m_lightList.begin(); it!=m_lightList.end(); ++it)
    {
    ...
    int rand = Eigen::internal::random<int>(1, x);
    Photon p = m_areaLightList[rand]->samplePhoton();
    return p;
