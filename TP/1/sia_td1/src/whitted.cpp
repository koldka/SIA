    ...
    Color3f Li(const Scene *scene, const Ray &ray) const {
        ...
        if (hit.foundIntersection())
        {
            ...
            for(LightList::const_iterator it=lights.begin(); it!=lights.end(); ++it)
            {
                ...
                {
                    light = dynamic_cast<const AreaLight*>(*it);
                    // source étendue
                    float randx = Eigen::internal::random<float>(0,light->size()[0]);
                    float randy = Eigen::internal::random<float>(0,light->size()[1]);

                    randPoint = light->position();
                    randPoint += light->uVec() * light->size().x()*randx;
                    randPoint += light->vVec() * light->size().y()*randy;

                    lightDir = randPoint-pos;
                ...
                if(dynamic_cast<const AreaLight*>(*it))
                {
                    radiance += light->intensity(pos,randPoint) * cos_term * brdf * attenuation;
                }
                else
                {
                 radiance += (*it)->intensity(pos) * cos_term * brdf * attenuation;
