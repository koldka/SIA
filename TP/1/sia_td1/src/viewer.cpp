...
void renderBlock(Scene* scene, ImageBlock& block)
{
  ...
  uint sampleCount = camera->sampleCount();
      ...
      for (int x=0; x<size.x(); ++x) {
          ...
          for(int i=0; i<sampleCount; i++){
              for(int j=0; j<sampleCount; j++){
                  ...
                  float randy = Eigen::internal::random<float>(0,1); /* Pour le jitter*/
                  if(!camera->isStratified()){
                      dx = (i+0.5)/sampleCount;
                      dy = (j+0.5)/sampleCount;
                  }else{
                      dx = (i+randx)/sampleCount;
                      dy = (j+randy)/sampleCount;
                  }
                  ...
                  c += integrator->Li(scene,ray);


              }
          }
          block.put(Vector2f(x+offset[0], y+offset[1]), c/(sampleCount*sampleCount));
      }
  }
}
