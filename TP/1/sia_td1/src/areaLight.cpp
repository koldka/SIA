...
Color3f AreaLight::intensity(const Point3f &x, const Point3f &y) const {
    ...
    if(m_texture) {
        Vector3f p = y-position();
        Vector2f size = this->size();

        float u = p.dot(uVec()/uVec().norm());
        float v = p.dot(vVec()/vVec().norm());

        int w_texture = (*m_texture).cols();
        int h_texture = (*m_texture).rows();

        float x_texture = u*w_texture/size[0];
        float y_texture = v*h_texture/size[1];

        intensity = (*m_texture)(x_texture, y_texture);
        // TODO
    }
