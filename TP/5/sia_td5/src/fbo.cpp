...
void FBO::init(int width, int height)
{
    ...
    glGenRenderbuffers(1, &_rbId); // on crée ça pour stocker la depth map
    glBindRenderbuffer(GL_RENDERBUFFER, _rbId);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F ,width, height);


    //5. attach the depth buffer to FBO depth attachment point
    // TODO
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, _rbId);
...
void FBO::savePNG(const std::string &name)
{
    ...
    glBindTexture(GL_TEXTURE_2D, renderedTexture[1]);
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
     raw_data = reinterpret_cast<unsigned char*>(data);
    ...
    SOIL_save_image((DATA_DIR"/" + name+"_normal.png").c_str(),SOIL_SAVE_TYPE_PNG,_width,_height,4,raw_data);
    glBindTexture(GL_TEXTURE_2D,0);
}
