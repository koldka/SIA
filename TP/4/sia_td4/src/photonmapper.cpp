...
public:
    ...
    void preprocess(const Scene *scene)
    {
        ...
        while(m_photonMap->size() < m_photonCount){
            ...
            float albedo = -1.0;
            float pdf;
            while ((albedo == -1.0 || albedo > Eigen::internal::random<float>(0, 1))&& m_photonMap->size() < m_photonCount && photon.getPower().mean() > Epsilon){
                ...
                albedo = color.mean()/ photon.getPower().mean();
                if(material->specularSamplingWeight() == 0){
                    m_photonMap->push_back(Photon(ray.at(hit.t()), -ray.direction, photon.getPower()));
                }
               photon = Photon(ray.at(hit.t()), r, color * albedo);
            }
        }
    ...
    Color3f Li(const Scene *scene, const Ray & ray) const
    {
        ...
        if (hit.foundIntersection())
        {
            ...
            if(material->specularSamplingWeight() == 0) {
                  ...
                  //Looking for photons within the radius
